...
\begin{enumerate}[label=\textbf{Questão \arabic*:}, leftmargin=*]
    ...
    % --- Questão 9 ---
    \item \textbf{Faixa de Operação da Tensão de Entrada}
    \begin{itemize}
        \item $I_L = 8\text{ V}/1,2\text{ k}\Omega \approx 6,67\text{ mA}$.
        \item $V_{i,min}$ ocorre quando o Zener começa a conduzir ($I_Z \approx 0$).
        \item $I_{Zmax} = 50\text{ mA} \implies I_{Rmax} = 56,67\text{ mA} \implies V_{i,max} = 8 + (56,67\text{m} \cdot 220) \approx \mathbf{20,47\text{ V}}$.
    \end{itemize}
